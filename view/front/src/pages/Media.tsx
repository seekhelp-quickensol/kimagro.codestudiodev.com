...
export default function Media() {
  ...
  const { mediaData, loading, error, refetch } = useMedia(activeTab) as UseMediaResult;
  const { categoryMedia, loading: categoryLoading, fetchCategoryMedia } = useMediaByCategory() as UseMediaByCategoryResult;

  // Filter categories based on current tab
  const filteredCategories: Category[] = activeTab === "All"
    ? mediaData.categories
    : mediaData.categories.filter((cat) => cat.type === activeTab);

  // Get media items for selected category
  const selectedCategoryMedia: MediaItem[] = selectedCategory ? categoryMedia : [];
  ...
  const newsMedia: MediaItem[] = selectedCategoryMedia.filter((item) => item.type === "news");

  const videoMedia: MediaItem[] = selectedCategoryMedia.filter((item) => item.type === "videos");
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`pb-1 text-sm md:text-base capitalize ${
                activeTab === tab ? "font-semibold border-b-2 border-black" : "text-gray-500"
              }`}
            >
              {tab}
            </button>
        ...
        {!selectedCategory && (
          <div className="my-12 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredCategories.map((category) => (
              ...
              >
                {currentLang === "hi" ? category.title_hindi : category.title_english}
              </button>
            ))}

            {filteredCategories.length === 0 && (
              <div className="col-span-full text-center py-12 text-gray-500">
                No categories found for this media type.
              </div>
            )}
          </div>
        )}
                  ...
                  >
                    <div className="relative w-full h-56 bg-black flex items-center justify-center">
                      {playingVideo === item.src ? (
                        <video
                          src={`${import.meta.env.VITE_APP_BACKEND}uploads/videos/${item.src}`}
                  ...
                  >
                    {item.thumb && (
                      <div className="w-full h-56 bg-gray-200">
                        <img
                          src={`${import.meta.env.VITE_APP_BACKEND}uploads/images/${item.thumb}`}
                          alt="news-thumb"
                          className="w-full h-full object-cover"
                        />
                      </div>
